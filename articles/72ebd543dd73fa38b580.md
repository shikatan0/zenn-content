...
この記事は、ScrapboxのUserCSSを変更した際のメモ書きを纏めたものです。
...
この記事では個人的なメモツールとしての運用を想定しています。

# UserCSS

自分のプロジェクトにCSSを適用できる機能。
https://scrapbox.io/help-jp/UserCSS

# フォントを変更

:::details font-family規定値一覧
```txt
|                    html | sans-serif                                             |
...
VSCodeで記述した`.sass`をCSSに変換してScrapboxのsettingsにコピペしている。
...
```sass
@import "../modules/読み込むsassファイル名1.sass"
@import "../modules/読み込むsassファイル名2.sass"
```
